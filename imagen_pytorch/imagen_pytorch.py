...
class ChanRMSNorm1d(nn.Module):
    """
    For 1D input (x). It is different from mean variance normalization
    """
    def __init__(self, dim):
        super().__init__()
        self.scale = dim ** 0.5 # avoid cummulative variance as dim increases
        self.gamma = nn.Parameter(torch.ones(dim, 1)) # Make it a trainable parameter

    def forward(self, x):
        return F.normalize(x, dim = 1) * self.scale * self.gamma # F.normmalize is more efficient and stable

class LayerNorm(nn.Module): ## Seems mean to for 1D input
...
class PixelShuffleUpsample1d(nn.Module):
    ...
    def init_conv_(self, conv):
        ...
        conv_weight = torch.empty(o // 2, i, l)
        nn.init.kaiming_uniform_(conv_weight)
...
class Block1d(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.norm = ChanRMSNorm1d(dim) if norm else Identity()
        self.activation = nn.SiLU()
...
class ResnetBlock1d(nn.Module):
    def __init__(
        ...
        **attn_kwargs
        ...
        if exists(cond_dim):
            attn_klass = CrossAttention if not linear_attn else LinearCrossAttention # These are not modified yet
    ...
    def forward(self, x, time_emb = None, cond = None):
        ...
        scale_shift = None
        if exists(self.time_mlp) and exists(time_emb): #TODO: CHANGE TO 1D
            # time_emb = self.time_mlp(time_emb)
            # time_emb = rearrange(time_emb, 'b c -> b c 1 1')
            # scale_shift = time_emb.chunk(2, dim = 1)
            pass
        ...
        if exists(self.cross_attn): #TODO: CHANGE TO 1D
            # assert exists(cond)
            # h = rearrange(h, 'b c h w -> b h w c')
            # h, ps = pack([h], 'b * c')
            # h = self.cross_attn(h, context = cond) + h
...
class CrossEmbedLayer1d(nn.Module):
    def __init__(
        ...
        # calculate the dimension at each scale
        dim_scales = [int(dim_out / (2 ** i)) for i in range(1, num_scales)]
        dim_scales = [*dim_scales, dim_out - sum(dim_scales)] #TODO: Cannot understand this line
...
class Unet1d(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.init_conv = CrossEmbedLayer1d(init_channels, dim_out = init_dim, kernel_sizes = init_cross_embed_kernel_sizes, stride = 1) if init_cross_embed else nn.Conv1d(init_channels, init_dim, init_conv_kernel_size, padding = init_conv_kernel_size // 2)
        ...
        # self.to_time_hiddens = nn.Sequential(   # We comment out NN that are not used and checked for now
        #     sinu_pos_emb,
        #     nn.Linear(sinu_pos_emb_input_dim, time_cond_dim),
        #     nn.SiLU()
        # )
        ...
        for ind, ((dim_in, dim_out), layer_num_resnet_blocks, layer_attn, layer_attn_depth, layer_cross_attn, layer_use_linear_attn, layer_use_linear_cross_attn) in enumerate(zip(in_out, *layer_params)):
            ...
            self.downs.append(nn.ModuleList([
                pre_downsample,
                resnet_klass(current_dim, current_dim, cond_dim = layer_cond_dim, linear_attn = layer_use_linear_cross_attn, time_cond_dim = time_cond_dim),
                nn.ModuleList([ResnetBlock1d(current_dim, current_dim, time_cond_dim = time_cond_dim, use_gca = use_global_context_attn) for _ in range(layer_num_resnet_blocks)]),
                transformer_block_klass(dim = current_dim, depth = layer_attn_depth, ff_mult = ff_mult, context_dim = cond_dim, **attn_kwargs),
                post_downsample
            ]))
        ...
        self.mid_block1 = ResnetBlock1d(mid_dim, mid_dim, cond_dim = cond_dim, time_cond_dim = time_cond_dim)
        self.mid_attn = None if attend_at_middle else None # TransformerBlock(mid_dim, depth = layer_mid_attns_depth, **attn_kwargs) if attend_at_middle else None
        self.mid_block2 = ResnetBlock1d(mid_dim, mid_dim, cond_dim = cond_dim, time_cond_dim = time_cond_dim)

        # upsample klass

        upsample_klass = Upsample if not pixel_shuffle_upsample else PixelShuffleUpsample1d

        # upsampling layers

        upsample_fmap_dims = []

        # if out_dim is not None: #Added when the output dimension is specified and not equals to initial dimension
        ...
        for ind, ((dim_in, dim_out), layer_num_resnet_blocks, layer_attn, layer_attn_depth, layer_cross_attn, layer_use_linear_attn, layer_use_linear_cross_attn) in enumerate(zip(reversed(in_out), *reversed_layer_params)):
            ...
            self.ups.append(nn.ModuleList([
                resnet_klass(dim_out + skip_connect_dim, dim_out, cond_dim = layer_cond_dim, linear_attn = layer_use_linear_cross_attn, time_cond_dim = time_cond_dim),
                nn.ModuleList([ResnetBlock1d(dim_out + skip_connect_dim, dim_out, time_cond_dim = time_cond_dim, use_gca = use_global_context_attn) for _ in range(layer_num_resnet_blocks)]),
                transformer_block_klass(dim = dim_out, depth = layer_attn_depth, ff_mult = ff_mult, context_dim = cond_dim, **attn_kwargs),
                upsample_klass(dim_out, dim_in) if not is_last or memory_efficient else Identity()
            ]))
        ...
        self.final_res_block = ResnetBlock1d(final_conv_dim, dim, time_cond_dim = time_cond_dim, use_gca = True) if final_resnet_block else None
